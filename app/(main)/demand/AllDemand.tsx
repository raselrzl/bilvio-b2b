...
import { saveDemandNote, setDemandStatusAction } from "@/app/actions";
import DemandActionDialog from "@/app/(main)/demand/DemandActionDialog";
import DemandNoteInput from "@/components/general/DemandNoteInput";
import DemandViewDialog from "./DemandViewDialog";
...
export default function AllDemands({
  ...
  {/* VIEW DETAILS */}
  <DemandViewDialog demand={demand} />

  {/* SET AS DRAFT */}
  <DemandActionDialog
    demandId={demand.id}
    label="Mark as Draft"
    action={() => setDemandStatusAction(demand.id, "DRAFT")}
  >
    <NotebookPen className="bg-gray-100 p-1 h-8 w-8 cursor-pointer rounded-xs" />
  </DemandActionDialog>

  {/* MESSAGE */}
  {demand.lastNote ? (
  <MessageViewDialog note={demand.lastNote} />
) : (
  <Link
    href={`/buyer/messages?demandId=${demand.id}`}
    className="bg-gray-100 p-1 h-8 w-8 cursor-pointer rounded-xs text-gray-700 hover:bg-gray-200"
  >
    <MessageCircle />
  </Link>
)}
  ...
  <DemandActionDialog
    demandId={demand.id}
    label="Save Demand"
    action={() => setDemandStatusAction(demand.id, "SAVED")}
  >
    <Save className="bg-gray-100 p-1 h-8 w-8 cursor-pointer rounded-xs" />
  </DemandActionDialog>
