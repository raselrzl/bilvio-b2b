...
import { Order, Product } from "./page";
...
export default function OrdersClient({ orders }: { orders: Order[] }) {
  ...
  const getReactionColor = (
    product: Product,
    type: "LIKE" | "UP" | "DOWN" | "SAVE"
  ) => {
    return product.reactions?.some((r) => r.reaction === type)
      ? "text-red-600" // active color if reaction exists
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <ThumbsUp
                        className={`h-5 w-5 ${getReactionColor(p, "UP")}`}
                      />
                      <ThumbsDown
                        className={`h-5 w-5 ${getReactionColor(p, "DOWN")}`}
                      />
                      <ClockPlus
                  ...
                  <div className="items-end justify-end text-end text-gray-600 text-sm capitalize">
                    <p>
                      Estimated transport cost {formatNumber(p.transportCost)}{" "}
                      SEK NET
                    </p>
                  </div>
